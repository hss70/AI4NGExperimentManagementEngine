...
## Runtime Behaviour: Resolving the Current Occurrence

The participant mobile client does **not** pre-generate all session occurrences.  
Instead, occurrences are **resolved and created on demand** using the protocol.
...
1. **Resume Existing**
   - If a participant has an `IN_PROGRESS` occurrence → return it.

2. **Required Protocol Session**
   - Load `PROTOCOL_SESSION` items for the experiment.
   - Evaluate cadence rules:
     - `ONCE`
     - `DAILY`
     - `WEEKLY`
   - Determine if an occurrence for the required cadence key exists.

3. **Create Required Occurrence**
   If no occurrence exists but protocol requires one, create:

```
PK = OCCURRENCE#{experimentId}#{participantId}
SK = {occurrenceKey}
```

Where:

| Cadence | Occurrence Key |
|-------|------|
| Once | `FIRST` |
...
4. **Optional Sessions**
If no required protocol session is due, the API returns **optional actions**.

These allow the participant to create:

```
OPTIONAL#<timestamp>#<shortGuid>
```

Example:

```
OPTIONAL#2026-03-16T15:16:15Z#efbe2c
```
...
| Status | Meaning |
|------|------|
| `scheduled` | Occurrence created but not started |
| `in_progress` | Participant has started session |
| `completed` | All required tasks finished |
| `cancelled` | Session aborted or invalidated |

---

## Task Execution Model

Each occurrence snapshots the task sequence used for that run.
...
| Field | Meaning |
|-----|-----|
| order | Task sequence position |
| taskKey | Reference to task definition |
| status | pending / started / completed / skipped |
| startedAt | When task began |
| endedAt | When task finished |
...
## Optional / Freeplay Sessions

Participants may create sessions outside the protocol.
...
- additional practice
- demo sessions
- recovery after EEG cap removal
...
isRequired = false
source = ParticipantInitiated
```

They use a **session type template** but are not enforced by protocol.

---

## Sync Model

Mobile clients sync using:
...
Clients only download updates where:
...
Protocol sessions define **rules**, not pre-generated schedules.

This allows:

- missed days
- irregular usage
- participant-specific pacing

### Deterministic Occurrence Keys

Required sessions use deterministic keys:
...
This prevents duplicate required sessions.

Optional sessions use random suffixes.

### Resumable Sessions

Task state is persisted per occurrence allowing:
...
Participants can complete tasks offline and sync results later.
...
- classifier version tracking per occurrence
- EEG hardware metadata
- protocol gating (e.g., classifier accuracy threshold)
- adaptive protocol sessions
- per-participant schedule offsets
- timezone-aware cadence windows
